...
//
// If you read this and feel like this is a whacky hack, then you'd be completely right.
// There's many better options, like redirecting all WriteConsoleInput calls to per-process input buffers.
...
void InputBuffer::_WriteBuffer(const std::span<const INPUT_RECORD>& inEvents, _Out_ size_t& eventsWritten, _Out_ bool& setWaitEvent)
{
    ...
    for (const auto& inEvent : inEvents)
    {
        ...
        if (vtInputMode)
        {
            ...
            const auto handled = _termInput.HandleKey(inEvent);
            if (handled)
            {
                eventsWritten++;
                continue;
...
// Return Value:
// - <none>
void InputBuffer::_HandleTerminalInputCallback(const std::wstring_view& text)
{
    try
    {
